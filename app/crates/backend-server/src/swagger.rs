...
pub fn swagger_ui(config: &Config) -> SwaggerUi {
    ...
    // Create the Signature security scheme (using header-based API key)
    let signature_scheme = SecurityScheme::ApiKey(ApiKey::Header(
        ApiKeyValue::new("x-auth-signature"),
    ));
